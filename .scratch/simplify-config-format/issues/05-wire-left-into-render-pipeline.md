...
- [ ] `generate-keymap` with a `:left`-only layer produces the same rendered keymap as the old `:bindings` full-grid equivalent.
- [ ] `generate-keymap` with `:right-override` produces the same rendered keymap as a full-grid `:bindings` with those cells changed.
- [ ] Combo-layers lacking `:row-widths` still render correctly by pulling from `:keyboard`.
- [ ] Old tile/placement tests are either removed or replaced with equivalent `:left` tests (intermediate state is fine — wide deletion happens in ticket 07).
